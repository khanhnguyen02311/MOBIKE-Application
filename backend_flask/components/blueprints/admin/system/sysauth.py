...
bpsysadminauth = Blueprint('bpsysadminauth', __name__)
...
@bpsysadminauth.route('/signin', methods = ['POST'])
def signin():
   ...
   try:
      ...
      acc = Session.query(dbm.Account).filter(dbm.Account.Email==username_or_email, dbm.Account.ID_Permission==1).first() or \
            Session.query(dbm.Account).filter(dbm.Account.Username==username_or_email, dbm.Account.Account_type==1).first()
