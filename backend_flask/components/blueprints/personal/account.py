...
def getinfo2():
   ...
   Session = new_Scoped_session()
   try:
      acc = Session.query(dbm.Account.ID, dbm.AccountInfo.Name, dbm.AccountInfo.Phone_number, dbm.AccountInfo.Identification_number, dbm.AccountInfo.Birthdate).join(dbm.AccountInfo, dbm.Account.ID == dbm.AccountInfo.ID).get(current_user['ID'])
      if acc == None:
         Session.close()
         return jsonify({"message": "Incompleted", "error": "Account not found", "info": ""})
      # acc_info = Session.query(dbm.AccountInfo).get(acc.ID_AccountInfo)
