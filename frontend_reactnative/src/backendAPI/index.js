...
export const isUsernameExist = async (username: String) => {
  ...
  const response = await HttpRequest.GetRequest(
    'gets/isusernameexists/' + username,
  );
  return response.exists;
};
...
export const GetPersonalInfo = async () => {
  let token = getToken();
  const infoResponse = await HttpRequest.ProtectedGetRequest(
    'personal/info/get',
    token,
  );
  const addressesResponse = await HttpRequest.ProtectedGetRequest(
    'personal/address/get',
    token,
  );
  ...
  if (infoResponse && addressesResponse) {
    if (
      infoResponse.msg == 'Completed' &&
      addressesResponse.msg == 'Completed'
    ) {
...
export const SetPersonalInfo = async (
  ...
  const token = getToken();
  const res = await HttpRequest.ProtectedPostRequest(
    'personal/info/set',
    body,
    token,
  );
...
export const SetIdentityImage = async (
  frontImage: Object,
  backImage: Object,
) => {
  const token = getToken();
...
export const SetPersonalAddress = async (addresses: Array) => {
  ...
  addresses.forEach(address => {
    if (!address.IsTemporary && address.IsDeleted) {
      ...
      pros.push(
        HttpRequest.ProtectedDeleteRequest(
          'personal/address/del/' + address.ID,
          token,
        ),
      );
      ...
      pros.push(
        HttpRequest.ProtectedPutRequest(
          'personal/address/set/' + address.ID,
          body,
          token,
        ),
      );
      ...
      pros.push(
        HttpRequest.ProtectedPostRequest('personal/address/add', body, token),
      );
    }
  });
...
export const UploadPost = async (
  ...
  if (imageResponses.every(imageResponse => imageResponse.msg == 'Completed')) {
    ...
    const vehicleInfoResponse = await HttpRequest.ProtectedPostRequest(
      'personal/post/vehicle/new',
      body,
      token,
    );
    console.log(
      'Vehicle info response: ' + JSON.stringify(vehicleInfoResponse),
    );
    if (vehicleInfoResponse.msg == 'Completed') {
      ...
      const postInfoResponse = await HttpRequest.ProtectedPostRequest(
        'personal/post/new',
        body,
        token,
      );
...
export const GetPersonalPost = async () => {
  const token = getToken();
  const postResponse = await HttpRequest.ProtectedGetRequest(
    'personal/post/all',
    token,
  );
...
export const GetPost = async ID => {
  if (!IsUsingDatabase) {
    ...
    const address = MockData.Address.find(
      address => address.ID == post.ID_Address,
    );
    ...
    const vehicleInfo = MockData.VehicleInfo.find(
      vehicleInfo => vehicleInfo.ID == post.ID_VehicleInfo,
    );
    return {
      address: address,
      post: {
...
export const PostFilter = (
  ...
  const argsString = Object.keys(args).map(key => {
    if (args[key]) {
      return `${key}=${args[key]}`;
    }
  });
...
export const GetAllPosts = async (args: string) => {
  ...
  const postResponse = await HttpRequest.GetRequest(
    'search/post/all' + (args ? '?' + args : ''),
  );
  if (postResponse.msg == 'Completed') {
    return postResponse.info;
  }
...
export const LikePost = async ID => {
  const token = getToken();
  const likeResponse = await HttpRequest.ProtectedPostRequest(
    'personal/like/' + ID,
    {},
    token,
  );
...
export const UnlikePost = async ID => {
  const token = getToken();
  const unlikeResponse = await HttpRequest.ProtectedDeleteRequest(
    'personal/like/' + ID,
    token,
  );
  await UpdateLikedPost();
...
export const RatePost = async (PostID, ratingPoint, content) => {
  ...
  const rateResponse = await HttpRequest.ProtectedPostRequest(
    'personal/rate/' + PostID,
    body,
    token,
  );
...
export const DeactivatePost = async ID => {
  const token = getToken();
  const deactivateResponse = await HttpRequest.ProtectedPostRequest(
    'personal/post/deactivate' + ID,
    token,
  );
...
export const SoldPost = async ID => {
  const token = getToken();
  const soldResponse = await HttpRequest.ProtectedPostRequest(
    'personal/post/sold' + ID,
    token,
  );
...
export const AppAdminGetInactivePost = async () => {
  const token = getToken();
  const postResponse = await HttpRequest.ProtectedGetRequest(
    'admin/application/inactiveposts',
    token,
  );
  if (postResponse.msg == 'Completed') {
    return postResponse.info;
...
export const AppAdminSetStatus = async (PostID, status, info) => {
  ...
  const postResponse = await HttpRequest.ProtectedPostRequest(
    'admin/application/status/set',
    body,
    token,
  );
