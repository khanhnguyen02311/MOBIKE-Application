...
import {
  ...
  setWards,
  setTree,
} from '../../redux/clientDatabase/location';
...
export const init = async () => {
  try {
    let isCientDatabaseInitialized = await AsyncStorage.getItem(
      'ClientDatabaseInitted',
    );
...
const updateAndLoadClientDatabase = async () => {
  try {
    await Promise.all([
      ...
      console.log(
        'Client database: ' +
          Store.getState().locations.Cities.length +
          ' cities loaded',
      );
      Store.dispatch(setDistricts(res[0].Districts));
      console.log(
        'Client database: ' +
          Store.getState().locations.Districts.length +
          ' districts loaded',
      );
      Store.dispatch(setWards(res[0].Wards));
      console.log(
        'Client database: ' +
          Store.getState().locations.Wards.length +
          ' wards loaded',
      );
      ...
      console.log(
        'Client database: ' +
          Store.getState().permissions.length +
          ' permissions loaded',
      );
      Store.dispatch(setImageTypes(res[2]));
      console.log(
        'Client database: ' +
          Store.getState().imageTypes.length +
          ' image types loaded',
      );
      Store.dispatch(setVehicleBrands(res[3]));
      console.log(
        'Client database: ' +
          Store.getState().vehicleModels.VehicleBrands.length +
          ' vehicle brands loaded',
      );
      Store.dispatch(setVehicleLineUps(res[4]));
      console.log(
        'Client database: ' +
          Store.getState().vehicleModels.VehicleLineUps.length +
          ' vehicle lineups loaded',
      );
      Store.dispatch(setVehicleTypes(res[5]));
      console.log(
        'Client database: ' +
          Store.getState().vehicleTypes.length +
          ' vehicle types loaded',
      );
      Store.dispatch(setVehicleConditions(res[6]));
      console.log(
        'Client database: ' +
          Store.getState().vehicleConditions.length +
          ' vehicle conditions loaded',
      );
      Store.dispatch(setColors(res[7]));
      console.log(
        'Client database: ' + Store.getState().colors.length + ' colors loaded',
      );
    });
...
const updateLocations = async () => {
  ...
  try {
    ...
    const localLocationsVersion = JSON.parse(
      await AsyncStorage.getItem('ClientDatabaseVersion'),
    ).Locations;
    // console.log('Local locations version: ' + localLocationsVersion);
    console.log(
      'Client database: ' + localLocationsVersion + ' locations version',
    );
    ...
    for (let i = 0; i < cities.length; i++) {
      let city = cities[i];
      let cityDistricts = districts.filter(
        district => district.ID_City == city.ID,
      );
      let cityDistrictsTree = [];
      for (let j = 0; j < cityDistricts.length; j++) {
        let district = cityDistricts[j];
        let districtWards = wards.filter(
          ward => ward.ID_District == district.ID,
        );
    ...
    console.log(
      'Locations updated, current version: ' + onlineLocationsVersion,
    );
...
const updateVehicleBrands = async () => {
  ...
  try {
    const onlineVehicleBrandsVersion = (await getVersion(VEHICLEBRANDS))
      .Version;
    // console.log('Online vehicle brands version: ' + onlineVehicleBrandsVersion);
    const localVehicleBrandsVersion = JSON.parse(
      await AsyncStorage.getItem('ClientDatabaseVersion'),
    ).VehicleBrands;
    // console.log('Local vehicle brands version: ' + localVehicleBrandsVersion);
    console.log(
      'Client database: ' +
        localVehicleBrandsVersion +
        ' vehicle brands version',
    );
    ...
    console.log(
      'Vehicle brands updated, current version: ' + onlineVehicleBrandsVersion,
    );
...
const updateVehicleLineups = async () => {
  ...
  try {
    const onlineVehicleLineupsVersion = (await getVersion(VEHICLELINEUPS))
      .Version;
    // console.log('Online vehicle lineups version: ' + onlineVehicleLineupsVersion);
    const localVehicleLineupsVersion = JSON.parse(
      await AsyncStorage.getItem('ClientDatabaseVersion'),
    ).VehicleLineups;
    // console.log('Local vehicle lineups version: ' + localVehicleLineupsVersion);
    console.log(
      'Client database: ' +
        localVehicleLineupsVersion +
        ' vehicle lineups version',
    );
    ...
    console.log(
      'Vehicle lineups updated, current version: ' +
        onlineVehicleLineupsVersion,
    );
...
const updateVehicleTypes = async () => {
  ...
  try {
    ...
    const localVehicleTypesVersion = JSON.parse(
      await AsyncStorage.getItem('ClientDatabaseVersion'),
    ).VehicleTypes;
    // console.log('Local vehicle types version: ' + localVehicleTypesVersion);
    console.log(
      'Client database: ' + localVehicleTypesVersion + ' vehicle types version',
    );
    ...
    console.log(
      'Vehicle types updated, current version: ' + onlineVehicleTypesVersion,
    );
...
const updateVehicleConditions = async () => {
  ...
  try {
    const onlineVehicleConditionsVersion = (await getVersion(VEHICLECONDITIONS))
      .Version;
    // console.log('Online vehicle conditions version: ' + onlineVehicleConditionsVersion);
    const localVehicleConditionsVersion = JSON.parse(
      await AsyncStorage.getItem('ClientDatabaseVersion'),
    ).VehicleConditions;
    // console.log('Local vehicle conditions version: ' + localVehicleConditionsVersion);
    console.log(
      'Client database: ' +
        localVehicleConditionsVersion +
        ' vehicle conditions version',
    );
    ...
    await AsyncStorage.setItem(
      VEHICLECONDITIONS,
      JSON.stringify(VehicleConditions),
    );
    ...
    console.log(
      'Vehicle conditions updated, current version: ' +
        onlineVehicleConditionsVersion,
    );
...
const updateColors = async () => {
  ...
  try {
    ...
    const localColorsVersion = JSON.parse(
      await AsyncStorage.getItem('ClientDatabaseVersion'),
    ).Colors;
...
const updatePermissions = async () => {
  ...
  try {
    ...
    const localPermissionsVersion = JSON.parse(
      await AsyncStorage.getItem('ClientDatabaseVersion'),
    ).Permissions;
    // console.log('Local permissions version: ' + localPermissionsVersion);
    console.log(
      'Client database: ' + localPermissionsVersion + ' permissions version',
    );
    ...
    console.log(
      'Permissions updated, current version: ' + onlinePermissionsVersion,
    );
...
const updateImageTypes = async () => {
  ...
  try {
    ...
    const localImageTypesVersion = JSON.parse(
      await AsyncStorage.getItem('ClientDatabaseVersion'),
    ).ImageTypes;
    // console.log('Local image types version: ' + localImageTypesVersion);
    console.log(
      'Client database: ' + localImageTypesVersion + ' image types version',
    );
    ...
    console.log(
      'Image types updated, current version: ' + onlineImageTypesVersion,
    );
...
export const print = async () => {
  try {
    let isClientDatabaseInitialized = await AsyncStorage.getItem(
      'ClientDatabaseInitted',
    );
